...
import {ANIME_ES_GENRE_ROLE} from 'src/constants/role';
...
export default {
    ...
    paginate: {
        ...
        type: HTTP_METHOD.GET,
        method: [
            roleMiddleware([ANIME_ES_GENRE_ROLE.READ], USER_LEVEL.ADMIN),
    ...
    paginateTrash: {
        ...
        type: HTTP_METHOD.GET,
        method: [
            roleMiddleware([ANIME_ES_GENRE_ROLE.RESTORE], USER_LEVEL.ADMIN),
            paginateMiddleware(paginateOptions),
            async function (req, res) {
    ...
    create: {
        ...
        method: [
            roleMiddleware([ANIME_ES_GENRE_ROLE.CREATE], USER_LEVEL.ADMIN),
    ...
    modify: {
        ...
        method: [
            roleMiddleware([ANIME_ES_GENRE_ROLE.UPDATE], USER_LEVEL.ADMIN),
            validateMiddleware(REQUEST_TYPE.PARAMS, rulesID),
    ...
    delete: {
        ...
        method: [
            roleMiddleware([ANIME_ES_GENRE_ROLE.DELETE], USER_LEVEL.ADMIN),
            validateMiddleware(REQUEST_TYPE.PARAMS, rulesID),
            async function (req, res) {
                const item = await genreRepository.findOneById(req.params.id);
                if (!item) return res.error(NOT_EXISTS);
