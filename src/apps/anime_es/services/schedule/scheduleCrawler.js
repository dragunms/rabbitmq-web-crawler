...
import {MQ_CRAWL_ANIME_ES_LATEST_EPISODE, SCHEDULE_CRAWL_ANIME_ES_LATEST_EPISODE} from 'src/apps/anime_es/constants/service';
...
(async () => {
    try {
        ...
        scheduleUnique.addSchedule(
            SCHEDULE_CRAWL_ANIME_ES_LATEST_EPISODE,
            _10_MINUTES,
            60,
            async function () {
                await messageQueue.send(MQ_CRAWL_ANIME_ES_LATEST_EPISODE, {}, false);
            },
            sendErrorMessage
        );
    } catch (err) {
        await sendErrorMessage('anime-es-schedule-crawler', err);
        await timeout(5000);
        process.exit(1);
    }
})();
