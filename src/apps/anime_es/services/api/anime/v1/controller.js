...
import {ANIME_ES_ANIME_ROLE, SYSTEM_ROLE} from 'src/constants/role';
import {USER_LEVEL} from 'src/constants/system';
import {rulesID, rulesSlug} from 'src/constants/validate';
import {MQ_CRAWL_ANIME_ES_ALL_ANIME, MQ_CRAWL_ANIME_ES_RECENT_ANIME_EPISODE} from 'src/apps/anime_es/constants/service';
...
export default {
    ...
    pingCrawlerAll: {
        ...
        method: [
            ...
            async function (req, res) {
                ...
                await messageQueue.send(MQ_CRAWL_ANIME_ES_ALL_ANIME);
    ...
    pingCrawlerRecent: {
        ...
        method: [
            ...
            async function (req, res) {
                const messageQueue = messageQueueProvider.getConnection(PROVIDER.RABBIT_MQ.SYSTEM);
                await messageQueue.send(MQ_CRAWL_ANIME_ES_RECENT_ANIME_EPISODE);
                return res.success(true);
            },
        ],
    },
    ...
    compare: {
        ...
        type: HTTP_METHOD.GET,
        method: [
            roleMiddleware([ANIME_ES_ANIME_ROLE.READ], USER_LEVEL.ADMIN),
    ...
    paginateTrash: {
        ...
        type: HTTP_METHOD.GET,
        method: [
            roleMiddleware([ANIME_ES_ANIME_ROLE.RESTORE], USER_LEVEL.ADMIN),
            paginateMiddleware(paginateOptions),
            async function (req, res) {
    ...
    create: {
        ...
        type: HTTP_METHOD.POST,
        method: [
            roleMiddleware([ANIME_ES_ANIME_ROLE.CREATE], USER_LEVEL.ADMIN),
    ...
    modify: {
        ...
        method: [
            roleMiddleware([ANIME_ES_ANIME_ROLE.UPDATE], USER_LEVEL.ADMIN),
            validateMiddleware(REQUEST_TYPE.PARAMS, rulesID),
    ...
    delete: {
        ...
        method: [
            roleMiddleware([ANIME_ES_ANIME_ROLE.DELETE], USER_LEVEL.ADMIN),
            validateMiddleware(REQUEST_TYPE.PARAMS, rulesID),
            async function (req, res) {
                const item = await animeRepository.findOneById(req.params.id);
                if (!item) return res.error(NOT_EXISTS);
