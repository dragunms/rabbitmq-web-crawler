...
import {DRAMA_EN_METADATA_ROLE} from 'src/constants/role';
import {USER_LEVEL} from 'src/constants/system';
import {rulesID} from 'src/constants/validate';

import metadataRepository from 'src/apps/drama_en/repositories/metadata/repository';
...
export default {
    ...
    paginate: {
        ...
        type: HTTP_METHOD.GET,
        method: [
            roleMiddleware([DRAMA_EN_METADATA_ROLE.READ], USER_LEVEL.ADMIN),
    ...
    paginateTrash: {
        ...
        type: HTTP_METHOD.GET,
        method: [
            roleMiddleware([DRAMA_EN_METADATA_ROLE.RESTORE], USER_LEVEL.ADMIN),
            paginateMiddleware(paginateOptions),
            async function (req, res) {
    ...
    create: {
        ...
        method: [
            roleMiddleware([DRAMA_EN_METADATA_ROLE.CREATE], USER_LEVEL.ADMIN),
    ...
    modify: {
        ...
        method: [
            roleMiddleware([DRAMA_EN_METADATA_ROLE.UPDATE], USER_LEVEL.ADMIN),
            validateMiddleware(REQUEST_TYPE.PARAMS, rulesID),
    ...
    delete: {
        ...
        method: [
            roleMiddleware([DRAMA_EN_METADATA_ROLE.DELETE], USER_LEVEL.ADMIN),
            validateMiddleware(REQUEST_TYPE.PARAMS, rulesID),
            async function (req, res) {
                const item = await metadataRepository.findOneById(req.params.id);
                if (!item) return res.error(NOT_EXISTS);
